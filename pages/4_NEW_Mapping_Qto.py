...
# """
# Mapping-driven quantity take-off + RC2 writer
...
#         # ifcopenshell.util.unit enums available in recent builds; hardcode if needed
...
#             break
#     if existing:
#         setattr(existing, value_attr, value)
#         existing.Unit = unit
#         return existing
...
#     type=("csv", "xls", "xlsx"),
#     help="A=classification • B=title • D=prop_template • E=quantity_type • F=unit_hint",
# )
...
# df_map["prop_template"]  = df_map["prop_template"].astype(str).str.strip()
...
# df_map = df_map.dropna(subset=["classification", "quantity_type", "prop_template"])
...
#         # if not matches:
#         #     continue
#         # Build unique classification numbers per element (preserve order)
#         seen, uniq_nums = set(), []
#         for c in get_classification_strings(el):
...
#         matches = [(num, map_dict[num]) for num in uniq_nums]
...
#         pset_rc2 = upsert_pset(model, el, "OEBBset_RC2_KE")

#         for idx, (num, row) in enumerate(matches, 1):
#             val = 1 if row.quantity_type == "COUNT_STK" else compute_quantity(row.quantity_type, v, f)
#             if val is None:
#                 continue

#             qto = upsert_qto_set(model, el, QTO_MAP.get(el.is_a(), "Qto_GenericBaseQuantities"))
#             if row.quantity_type == "COUNT_STK":
#                 qtype, attr, unit_obj = "IfcQuantityCount", "CountValue", None
#             elif row.quantity_type.startswith("VOLUME"):
#                 qtype, attr, unit_obj = "IfcQuantityVolume", "VolumeValue", unit_vol
#             elif row.quantity_type.startswith("AREA"):
#                 qtype, attr, unit_obj = "IfcQuantityArea", "AreaValue", unit_area
#             else:
#                 qtype, attr, unit_obj = "IfcQuantityLength", "LengthValue", unit_len

#             # qto.Quantities += (
#             #     model.create_entity(
#             #         qtype, Name=row.quantity_type, Unit=unit_obj, **{attr: val}
#             #     ),
#             # )
#             upsert_quantity(
#                 model, qset=qto, qtype=qtype, name=row.quantity_type,
#                 unit=unit_obj, value_attr=attr, value=val
#             )

#             # RC2 props
#             upsert_single_value(
#                 model, pset_rc2, f"{idx}0_StandardElement", f"{num} {row.title}", "IfcLabel"
#             )
#             upsert_single_value(
#                 model,
#                 pset_rc2,
#                 row.prop_template.replace("X", str(idx)),
#                 val,
#                 "IfcReal" if unit_obj else "IfcCountMeasure",
#                 unit=unit_obj,
#             )
